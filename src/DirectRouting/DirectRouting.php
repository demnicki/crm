 ...
 * */
{
    ...
    public function deleteController(){
        ...
        try {
            ...
            return 'Błedna nazwa kontrolera. Coś poszło nie tak.';
        }
    }
    /* Funkcja tworząca nowy kontroler. */
    public function addControlerInit($typeControl,$orLog, $nota){
    ...
    public function addContentHtml($idTemplate, $title, $content){
        $sqlContentHtml = "INSERT INTO `page_html` (`name_control`, `title`, `id_template`, `content`) VALUES ('".$this->_nameControl."', '".$title."', '".$idTemplate."', '".$content."')";
        $obj = new ServicesDatabase;
        $db_handle = $obj->connectDb($sqlContentHtml);
        unset($obj);
        return "Pomyślnie dodano treść HTML w kontrolerze".$this->_nameControl;
    ...
    public static function showAllController(){
        ...
        <td style="width: 20px"><a href="../_php/editcontrol?name='.$row['name_control'].'">Edytuj kontroler</a></td>
        <td style="width: 20px"><a href="../_php/delcontrol?name='.$row['name_control'].'">Usuń kontroler</a></td>
    </tr>';
        $lp++;
        }
        return $table.'</table>';
